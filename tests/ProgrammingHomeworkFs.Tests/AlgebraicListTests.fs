...
module OopListTests =
    ...
    let tests =
        testList "Types and functions tests" [
            testList "bubbleSort MyOOPList tests" [
                testCase "bubbleSort test -- random int MyOOPList"
                <| fun _ ->
                    let actualResult =
                        let lst = List(1, List(3, EmptyList()))
                        toAlgebraicList(bubbleSort lst)
                    Expect.equal actualResult (AlgebraicList.Construct(1, AlgebraicList.Construct(3, AlgebraicList.Empty)))
                ...
                <| fun _ ->
                    let actualResult =
                        ...
                        toAlgebraicList(bubbleSort lst)
                    Expect.equal actualResult (AlgebraicList.Construct("1", AlgebraicList.Construct("18", AlgebraicList.Empty)))
            ...
            testList "quickOOPSort MyOOPList property tests" [
                ...
                <| fun (lst : list<int>) ->
                    Expect.equal<| List.sort lst <|  fromOOPListToList (bubbleSort (fromListToOOPList lst)) <| "Unexpected result"
            ...
            testList "concatOOP MyOOPList tests" [
                ...
                <| fun _ ->
                     ...
                     Expect.equal actualResult (AlgebraicList.Construct("18", AlgebraicList.Construct("1", AlgebraicList.Construct("18", AlgebraicList.Construct("1", AlgebraicList.Empty)))))
                ...
                <| fun _ ->
                     let actualResult =
                        ...
                        let lst2 = EmptyList()
                        toAlgebraicList(concat lst1 lst2)
                     Expect.equal actualResult AlgebraicList.Empty
                         "concatOOP EmptyList() and EmptyList() as same as AlgebraicList.Empty"
            ]
        ]
