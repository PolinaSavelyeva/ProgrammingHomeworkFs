...
let rec fastExpo (basement: float) (exponent: int) : float = // exponent >= 0
    if basement = 0.0 && exponent = 0 then
        ...
        let body = fastExpo basement (exponent / 2) // сохраняем предыдущее значение рекурсивной функции
...
/// Функция inline возвращает разницу между максимальным и минимальным элементами массива
